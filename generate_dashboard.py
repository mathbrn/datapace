...
function ovSelect(idx){
  ...
  function ovMetric(label,value,borderColor,sub){return'<div class="ov-stat" style="border:none;border-left:3px solid '+borderColor+';background:#141414;border-radius:0 8px 8px 0;"><div class="ov-stat-label">'+label+'</div><div class="ov-stat-value" style="font-size:28px;font-weight:700">'+value+'</div>'+(sub||'')+'</div>';}
  ...
  if(menVal){stats+=ovMetric('Record homme'+(menYr?' ('+menYr+')':''),menVal,'#22C55E');}
  var wmVal=wr&&wr.women?wr.women:(td&&td.women?td.women:null);
  var wmYr=wr&&wr.womenYr?wr.womenYr:(td?td.yr:null);
  if(wmVal){stats+=ovMetric('Record femme'+(wmYr?' ('+wmYr+')':''),wmVal,'#FCDB00');}
  stats+='</div>';
  ...
  if(wh.women.length>=2){
    ...
    charts2Html+='<div class="ov-chart-box"><div class="ov-chart-label">Record Femme</div><div style="text-align:center;padding:2rem 0"><div style="font-size:22px;font-weight:700;color:#FCDB00">'+wh.women[0].time+'</div><div style="font-size:12px;color:var(--text3);margin-top:4px">en '+wh.women[0].yr+'</div></div></div>';
  }
  ...
  if(wcv&&wh.women.length>=2){
    ovChartW=new Chart(wcv,{type:'line',
      data:{labels:wh.women.map(function(e){return e.yr;}),datasets:[{data:wh.women.map(function(e){return e.sec/60;}),borderColor:'#FCDB00',backgroundColor:'rgba(252,219,0,0.08)',tension:0.3,pointRadius:4,pointBackgroundColor:'#FCDB00',borderWidth:2,fill:true}]},
      options:{responsive:true,maintainAspectRatio:false,
