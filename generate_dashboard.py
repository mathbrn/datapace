...
function ovSelect(idx){
  ...
  // Finishers: always show if we have at least 1
  if(lastEd){
    var finDelta='';
    if(prevEd&&prevEd.v>0){var d=((lastEd.v-prevEd.v)/prevEd.v*100);finDelta='<div style="font-size:11px;margin-top:2px;color:'+(d>=0?'#22C55E':'#EF4444')+'">'+(d>=0?'+':'')+d.toFixed(1)+'% vs '+prevEd.yr+'</div>';}
    stats+='<div class="ov-stat"><div class="ov-stat-label">Finishers ('+lastEd.yr+')</div><div class="ov-stat-value">'+fmtFull(lastEd.v)+'</div>'+finDelta+'</div>';
  }
  // Temps moyen
  if(td&&td.avg){stats+='<div class="ov-stat"><div class="ov-stat-label">Temps moyen ('+td.yr+')</div><div class="ov-stat-value" style="font-size:14px">'+td.avg+'</div></div>';}
  // Record homme
  var menVal=wr&&wr.men?wr.men:(td&&td.men?td.men:null);
  var menYr=wr&&wr.menYr?wr.menYr:(td?td.yr:null);
  if(menVal){stats+='<div class="ov-stat"><div class="ov-stat-label">Record homme'+(menYr?' ('+menYr+')':'')+'</div><div class="ov-stat-value" style="font-size:14px;color:#9B6FFF">'+menVal+'</div></div>';}
  // Record femme
  var wmVal=wr&&wr.women?wr.women:(td&&td.women?td.women:null);
  var wmYr=wr&&wr.womenYr?wr.womenYr:(td?td.yr:null);
  if(wmVal){stats+='<div class="ov-stat"><div class="ov-stat-label">Record femme'+(wmYr?' ('+wmYr+')':'')+'</div><div class="ov-stat-value" style="font-size:14px;color:#FF8A50">'+wmVal+'</div></div>';}
  stats+='</div>';
  ...
  if(wh.men.length>=2){
    ...
    charts2Html+='<div class="ov-chart-box"><div class="ov-chart-label">Record Homme</div><div style="text-align:center;padding:2rem 0"><div style="font-size:22px;font-weight:700;color:#9B6FFF">'+wh.men[0].time+'</div><div style="font-size:12px;color:var(--text3);margin-top:4px">en '+wh.men[0].yr+'</div></div></div>';
  }
  ...
  if(wh.women.length>=2){
    ...
    charts2Html+='<div class="ov-chart-box"><div class="ov-chart-label">Record Femme</div><div style="text-align:center;padding:2rem 0"><div style="font-size:22px;font-weight:700;color:#FF8A50">'+wh.women[0].time+'</div><div style="font-size:12px;color:var(--text3);margin-top:4px">en '+wh.women[0].yr+'</div></div></div>';
  }
  ...
  var badgeBg=ac+'18';var badgeCol=ac;
  ...
  if(fc&&nYears>=2){
    ovChartF=new Chart(fc,{type:'bar',
      data:{labels:positiveHistory.map(function(e){return e.yr;}),datasets:[{data:positiveHistory.map(function(e){return e.v;}),backgroundColor:ac+'99',borderRadius:3,borderSkipped:false}]},
      options:{responsive:true,maintainAspectRatio:false,
        ...
        scales:{x:{grid:{display:false},ticks:TICK,border:{color:mkBorder()}},y:{beginAtZero:true,grid:{color:mkGRID()},ticks:{color:mkTICK().color,font:{size:11},callback:function(v){return fmt(v);}},border:{color:mkBorder()}}}
      }
    });
  }
