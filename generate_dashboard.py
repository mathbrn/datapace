...
function updateInsights(){
  ...
  var maxAbsPct=Math.max.apply(null,withDelta.map(function(r){return Math.abs(r.pct);}));
  if(!maxAbsPct)maxAbsPct=1;
  function distBadge(d){var c={MARATHON:'#9B6FFF',SEMI:'#FF8A50','10KM':'#5CDFA0',AUTRE:'#F472B6'};var l={MARATHON:'Marathon',SEMI:'Semi','10KM':'10km',AUTRE:'Autre'};return'<span class="dp-badge" style="background:'+((c[d]||'#9B6FFF')+'18')+';color:'+(c[d]||'#9B6FFF')+';font-size:10px;padding:2px 8px;margin-right:8px;">'+(l[d]||d)+'</span>';}
  function renderList(arr,el){
    var html='';
    arr.forEach(function(r){
      var sign=r.pct>=0?'+':'';
      var isUp=r.pct>=0;
      var barW=Math.min(Math.abs(r.pct)/maxAbsPct*100,100).toFixed(1);
      var barCol=isUp?'rgba(34,197,94,.15)':'rgba(239,68,68,.15)';
...
function updateTrends(){
  ...
  if(legEl){
    var legHtml='';
    if(dist==='ALL'){
      legHtml+='<span class="leg-item"><span class="leg-dot" style="background:#9B6FFF"></span>Marathon</span>';
...
var _spBS={},_spActiveSector='ALL',_spActiveBrand=null,_spPillSectors=[],_spRegion='ALL',_spViewMode='brands',_spSortMode='exp';
...
function spRenderKpis(){
  ...
  var kpis=[
    [nBrands,'Marques','#22C55E',prev?trendHtml(nBrands,prev.brands,'#22C55E'):''],
    [nEvents,'\\u00c9v\\u00e9nements','#38BDF8',prev?trendHtml(nEvents,prev.events,'#38BDF8'):''],
...
function spFiltered(){
  ...
  if(_spSortMode==='events')list.sort(function(a,b){return b[1].events.length-a[1].events.length||b[1].exposure-a[1].exposure;});
  else list.sort(function(a,b){return b[1].exposure-a[1].exposure;});
  return list;
}
...
function spEventsData(){
  ...
  SP_PARTNERSHIPS.forEach(function(p){
    ...
    var active=(p.years||[]).some(function(y){return y>=pMinYr&&y<=pMaxYr;});
    if(!active)return;
    if(!evMap[p.event])evMap[p.event]={brands:[],types:{}};
    if(evMap[p.event].brands.indexOf(p.brand)<0)evMap[p.event].brands.push(p.brand);
    evMap[p.event].types[p.brand]=p.type;
  });
  return Object.entries(evMap).sort(function(a,b){return b[1].brands.length-a[1].brands.length;});
}
function spSelectEvent(evName){
  _spActiveBrand=evName;
  spRenderTreemap();
  // Show event sponsors in detail
  var evData=spEventsData().find(function(e){return e[0]===evName;});
  if(!evData)return;
  var ev=RAW.find(function(r){return r.r===evName;});
  var col=ev?colDist(ev):'#9B6FFF';
  var brands=evData[1].brands;
  var types=evData[1].types;
  var byType={title:[],premium:[],major:[],official:[],partner:[]};
  brands.forEach(function(b){var t=types[b]||'partner';(byType[t]||byType.partner).push(b);});
  var tConf={title:{label:'Titre',bg:col,text:'#fff'},premium:{label:'Premium',bg:col+'90',text:'#fff'},major:{label:'Majeur',bg:col+'60',text:'#fff'},official:{label:'Officiel',bg:col+'30',text:col},partner:{label:'Fournisseur',bg:'var(--bg3)',text:'var(--text2)'}};
  var evHtml='';
  ['title','premium','major','official','partner'].forEach(function(t){
    var items=byType[t];if(!items.length)return;
    var tc=tConf[t];
    evHtml+='<div style="margin-bottom:8px"><span style="display:inline-block;font-size:10px;padding:2px 8px;border-radius:3px;background:'+tc.bg+';color:'+tc.text+';font-weight:600;margin-bottom:4px">'+tc.label+'</span><div style="display:flex;flex-wrap:wrap;gap:3px;margin-top:3px">';
    items.forEach(function(b){
      var sec=(SP_BRANDS[b]||{}).sector||'Autre';
      var bCol=_spCols[sec]||'#9B6FFF';
      evHtml+='<span class="sp-evtag" style="cursor:pointer;border-color:'+bCol+'44" data-brand="'+b.replace(/"/g,'&quot;')+'" onclick="spToggleView(false);spSelect(this.dataset.brand)" title="'+b+' ('+sec+')">'+b+'</span>';
    });
    evHtml+='</div></div>';
  });
  var rawIdx=ev?RAW.indexOf(ev):-1;
  var navBtn=rawIdx>=0?'<div style="margin-top:8px"><span class="sp-evtag" style="cursor:pointer;color:'+col+';border-color:'+col+'44" onclick="biggestClick('+rawIdx+')">\\u2192 Vue d\\'ensemble</span></div>':'';
  var det=document.getElementById('sp-detail');
  det.innerHTML='<div style="min-width:130px"><div class="sp-detail-name">'+evName+'</div>'+(ev?'<div style="font-size:11px;color:var(--text3)">'+ev.c+' \\u00b7 '+ev.p+'</div>':'')+'</div>'
    +'<div class="sp-detail-stat"><div class="sp-detail-stat-num" style="color:'+col+'">'+brands.length+'</div><div class="sp-detail-stat-lbl">Sponsors</div></div>'
    +'<div style="flex:1;min-width:180px">'+evHtml+navBtn+'</div>';
  det.style.display='flex';
}
function spToggleView(toEvents){
  _spViewMode=toEvents?'events':'brands';
  _spActiveBrand=null;
  document.getElementById('sp-detail').style.display='none';
  var btns=document.querySelectorAll('.sp-view-btn');
  btns.forEach(function(b){b.classList.toggle('spc-active',b.dataset.view===_spViewMode);});
  spRenderList();spRenderTreemap();
  spAutoSelect();
}
function spAutoSelect(){
  if(_spViewMode==='brands'){
    var items=spFiltered();
    if(items.length)spSelect(items[0][0]);
  }else{
    var evs=spEventsData();
    if(evs.length)spSelectEvent(evs[0][0]);
  }
}
function spRenderTreemap(){
  var container=document.getElementById('sp-treemap');
  var hint=document.getElementById('sp-treemap-hint');
  Array.from(container.children).forEach(function(c){if(c!==hint&&!c.classList.contains('sp-view-toggle'))container.removeChild(c);});
  if(_spViewMode==='events'){
    var evItems=spEventsData().slice(0,50);
    if(!evItems.length)return;
    var W=container.clientWidth||700,H=container.clientHeight||350;
    var nodes=evItems.map(function(e){
      var ev=RAW.find(function(r){return r.r===e[0];});
      var evCol=ev?colDist(ev):'#9B6FFF';
      return{id:e[0],v:Math.max(e[1].brands.length,1),sec:'__ev_'+e[0]};
    });
    nodes.forEach(function(n){var ev=RAW.find(function(r){return r.r===n.id;});_spCols[n.sec]=ev?colDist(ev):'#9B6FFF';});
  ...
  function place(n,x,y,w,h){
    ...
    var isEvView=_spViewMode==='events';
    var act=isEvView?(_spActiveBrand===n.id):(_spActiveBrand===n.id);
    ...
    if(w>=200){
      ...
      if(w>100&&h>44)inner+='<span style="position:absolute;bottom:6px;left:7px;font-size:10px;color:var(--text3);pointer-events:none">'+spFmt(n.v)+(isEvView?' marques':' finishers')+'</span>';
    }else if(w>=100){
    ...
    d.innerHTML=inner;
    d.addEventListener('mouseenter',function(){if(act)return;this.style.background='var(--bg3)';this.style.borderColor=col+'88';});
    d.addEventListener('mouseleave',function(){if(act)return;this.style.background='var(--bg2)';this.style.borderColor='var(--border)';});
    d.addEventListener('click',function(){if(isEvView){spSelectEvent(n.id);}else{spSelect(n.id);}});
    d.title=n.id+' \\u2022 '+(isEvView?n.v+' marques':n.sec+' \\u2022 '+spFmt(n.v)+' finishers');
...
:root{--bg:#13131a;--bg2:#1a1a24;--bg3:#1f1f2e;--bg4:#25253a;--border:#25253a;--border2:#303048;--text:#f0f0f5;--text2:#8a8a9a;--text3:#55555f;--purple:#5C00D4;--purple2:#7B2FFF;--yellow:#FCDB00;--accent:#5C00D4;}
[data-theme="light"]{--bg:#f8f8f8;--bg2:#ffffff;--bg3:#f0f0f0;--bg4:#e8e8e8;--border:rgba(0,0,0,0.08);--border2:rgba(0,0,0,0.12);--text:#0a0a0a;--text2:#444;--text3:#888;--purple:#5C00D4;--purple2:#4800A8;--yellow:#A88F00;--accent:#5C00D4;}
[data-theme="light"]{scrollbar-color:rgba(0,0,0,0.15) transparent;}
[data-theme="light"] ::-webkit-scrollbar-thumb{background:rgba(0,0,0,0.15);}
[data-theme="light"] ::-webkit-scrollbar-thumb:hover{background:#5C00D4;}
...
select:focus{border-color:var(--purple);color:var(--text);box-shadow:0 0 0 3px rgba(92,0,212,.1);}
...
.search-wrap input:focus{border-color:var(--purple);box-shadow:0 0 0 3px rgba(92,0,212,.1);}
...
/* ── Light mode: badges ── */
[data-theme="light"] .badge-world{background:#5C00D415;color:#4800A8;}
...
<nav class="dash-nav">
  ...
  <div class="dash-nav-right">
    ...
    <a href="mailto:mathis.brun@orange.fr" title="Contact" style="color:var(--text3);transition:color .2s;display:flex;align-items:center;" onmouseover="this.style.color='var(--text)'" onmouseout="this.style.color='var(--text3)'"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="M22 4L12 13 2 4"/></svg></a>
  </div>
</nav>
<div class="dash-body">
<div class="tabs">
...
<div id="panel-insights" class="panel">
  ...
  <div style="margin-top:1.5rem;">
    <div class="ins-card-title" style="margin-bottom:1rem;">&#x1F4CA; Chiffres cles</div>
    <div class="metrics" id="ins-metrics"></div>
  </div>
</div>
  ...
  <div class="sp-layout">
    <div class="sp-sidebar">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;margin-top:4px;margin-bottom:2px">
        ...
        <div class="sp-sort-toggle" style="display:flex;gap:1px;font-size:9px">
          <span class="sp-sort-btn spc-active" data-sort="exp" onclick="spSetSort('exp')" style="cursor:pointer;padding:1px 5px;border-radius:3px">Finishers</span>
          <span class="sp-sort-btn" data-sort="events" onclick="spSetSort('events')" style="cursor:pointer;padding:1px 5px;border-radius:3px;color:var(--text3)">Events</span>
        </div>
      </div>
      <div class="sp-blist" id="sp-blist"></div>
    </div>
    <div class="sp-right">
      <div class="sp-treemap" id="sp-treemap" style="position:relative">
        <div class="sp-view-toggle" style="position:absolute;top:8px;right:8px;z-index:2;display:flex;gap:1px;background:var(--bg);border-radius:6px;padding:2px;border:1px solid var(--border)">
          <span class="sp-view-btn spc-active" data-view="brands" onclick="spToggleView(false)" style="cursor:pointer;font-size:10px;padding:3px 8px;border-radius:4px">Marques</span>
          <span class="sp-view-btn" data-view="events" onclick="spToggleView(true)" style="cursor:pointer;font-size:10px;padding:3px 8px;border-radius:4px;color:var(--text3)">Evenements</span>
        </div>
...
<footer class="dp-footer">
  ...
  <div class="dp-footer-copy">DataPace &copy; 2026 &middot; NEXT:RACE SASU</div>
</footer>"""
