...
function updateInsights(){
  ...
  // Fix 1: top5 growth = positive only, bot5 decline = negative only
  var top5=withDelta.filter(function(r){return r.pct>0;}).slice(0,5);
  var bot5=withDelta.filter(function(r){return r.pct<0;}).sort(function(a,b){return a.pct-b.pct;}).slice(0,5);
  var maxGrowthPct=top5.length?Math.max.apply(null,top5.map(function(r){return r.pct;})):1;
  var maxDeclinePct=bot5.length?Math.max.apply(null,bot5.map(function(r){return Math.abs(r.pct);})):1;
  ...
  function renderList(arr,el,maxPct,isGrowth){
    var html='';
    if(!arr.length){html='<div style="color:var(--text3);font-size:12px;padding:10px">Aucun</div>';}
    arr.forEach(function(r){
      var sign=r.pct>=0?'+':'';
      var barW=Math.min(Math.abs(r.pct)/maxPct*100,100).toFixed(1);
      var barCol=isGrowth?'rgba(34,197,94,.15)':'rgba(239,68,68,.15)';
      var txtCls=isGrowth?'ins-pct-up':'ins-pct-down';
  ...
  renderList(top5,'ins-growth',maxGrowthPct,true);
  renderList(bot5,'ins-decline',maxDeclinePct,false);
  // Key metrics
  ...
  // Median growth
  var sortedPcts=withDelta.map(function(r){return r.pct;}).sort(function(a,b){return a-b;});
  var medianGrowth=sortedPcts.length?(sortedPcts.length%2===1?sortedPcts[Math.floor(sortedPcts.length/2)]:(sortedPcts[sortedPcts.length/2-1]+sortedPcts[sortedPcts.length/2])/2):0;
  ...
  var bigCol=biggest?colDist(biggest):'var(--text)';
  document.getElementById('ins-metrics').innerHTML=
    '<div class="metric" style="grid-column:span 2"><div class="metric-label">Total finishers '+toYr+'</div><div class="metric-value" style="font-size:28px">'+fmtFull(totalFin)+'</div><div class="metric-sub">'+allWithTarget.length+' evenements</div></div>'
    +'<div class="metric"><div class="metric-label">Plus grand '+toYr+'</div><div class="metric-value" style="font-size:14px;font-weight:700;color:'+bigCol+'">'+(biggest?biggest.r:'-')+'</div><div class="metric-sub" style="color:'+bigCol+'">'+(biggest?fmtFull(hv(biggest,toYr))+' finishers':'-')+'</div></div>'
    +'<div class="metric"><div class="metric-label">Croissance (m\\u00e9diane)</div><div class="metric-value" style="color:'+(medianGrowth>=0?'#22C55E':'#EF4444')+'">'+(medianGrowth>=0?'+':'')+medianGrowth.toFixed(1)+'%</div><div class="metric-sub">'+fromYr+' \\u2192 '+toYr+'</div></div>'
    +'<div class="metric"><div class="metric-label">Tendance</div><div class="metric-value" style="font-size:13px"><span style="color:#22C55E">'+growing+' en croissance</span> <span style="color:var(--text3)">\\u00b7</span> <span style="color:#EF4444">'+declining+' en d\\u00e9clin</span></div></div>';
  // Global chart: total finishers per year
  var globalYrs=[];
  for(var y=fromYr;y<=toYr;y++)globalYrs.push(y);
  var globalTotals=globalYrs.map(function(yr){
    var total=0;
    RAW.forEach(function(r){
      if(distF!=='ALL'&&r.d!==distF)return;
      var v=(r.hist||{})[yr];
      if(v&&v>0)total+=v;
    });
    return total;
  });
  if(cInsGlobal)cInsGlobal.destroy();
  var ctx=document.getElementById('ins-global-chart');
  if(ctx){
    document.getElementById('ins-global-title').textContent='TOTAL FINISHERS TRACK\\u00c9S \\u00b7 '+fromYr+'-'+toYr;
    cInsGlobal=new Chart(ctx,{type:'line',data:{labels:globalYrs.map(String),datasets:[{data:globalTotals,borderColor:lc('#5C00D4'),backgroundColor:lc('#5C00D4')+'18',fill:true,tension:0.35,pointRadius:3,pointBackgroundColor:lc('#5C00D4'),borderWidth:2}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:mkTT({label:function(ctx){return' '+fmtFull(ctx.parsed.y)+' finishers';}})},scales:{x:{grid:{color:mkGRID()},ticks:{color:mkTICK().color,font:{size:10},maxRotation:0}},y:{beginAtZero:false,grid:{color:mkGRID()},ticks:{color:mkTICK().color,font:{size:10},callback:function(v){return fmt(v);}}}}}});
  }
}
...
<div id="panel-insights" class="panel">
  <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;margin-bottom:1.25rem;">
    <div class="controls" style="margin-bottom:0;">
      <div class="ctrl-group">
        <span class="ctrl-label">P&eacute;riode</span>
        <select id="ins-yr-from" onchange="updateInsights()"></select>
        <span style="color:var(--text3);font-size:11px;margin:0 4px">&rarr;</span>
  ...
  <div style="margin-bottom:1.25rem;">
    <div class="section-title" id="ins-global-title" style="font-size:10px;letter-spacing:.08em;margin-bottom:6px">TOTAL FINISHERS TRACK&Eacute;S</div>
    <div style="height:150px;position:relative"><canvas id="ins-global-chart"></canvas></div>
  </div>
  <div class="ins-grid">
    <div class="ins-card">
    ...
    <div class="ins-card">
      <div class="ins-card-title">TOP 5 &mdash; Plus fort d&eacute;clin</div>
      <div id="ins-decline"></div>
    </div>
  </div>
  <div style="margin-top:1.5rem;">
    <div class="ins-card-title" style="margin-bottom:1rem;font-size:10px;letter-spacing:.08em;text-transform:uppercase">CHIFFRES CL&Eacute;S</div>
